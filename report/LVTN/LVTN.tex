...
% Tùy chỉnh margin, đặt trước fancy
\usepackage[top=2cm,bottom=3cm,left=3cm,right=2cm,includeheadfoot,asymmetric]{geometry}
...
\lfoot{\textbf{Luận văn tốt nghiệp}} % Bottom left footer
...
Trực quan dữ liệu (\textit{Data visualization}), được xem như hướng đi trong tương lai của việc giao tiếp và truyền thông thông qua hình ảnh. Nó liên quan đến các công trình và nghiên cứu nhằm biểu diễn dữ liệu một cách trực quan, hay nói cách khác, được hiểu là "thông tin được trực quan hoá dưới dạng các lược đồ, bao gồm các thuộc tính và thông số đại diện cho các đơn vị thông tin\cite{dataviz_wiki}".
%----------------------------------%

%----------------------------------%
\myparagraph{Lịch sử hình thành}
Bắt đầu từ thế kỷ thứ 2 với việc sắp xếp dữ liệu vào các cột và các hàng, phát triển thành biểu diễn dưới dạng các đơn vị định lượng ban đầu trong thế kỷ 17\cite{wiki_history_1}. Theo \textit{Hiệp hội Thiết kế Tương tác} (\textit{The Interaction Design Foundation}), nhà triết học và toán học người Pháp \textit{René Descartes} đã đưa ra những nghiên cứu nền tảng đầu tiên làm bước đệm cho một kỹ sư người Scotland tên là \textit{William Playfair}. \textit{Descartes} đã phát triển một hệ trục toạ độ hai chiều để hiển thị các giá trị, vào những năm cuối thế kỷ 18, \textit{Playfair} đã nhìn thấy tiềm năng trong việc thể hiện đồ họa của dữ liệu định lượng\cite{wiki_history_1}.
...
\clearpage
%\NewPage
\section{Công trình liên quan \& Kiến thức nền tảng}
...
\item[•] \emph{Khả năng kiểm soát (Controllable):} C3.js cung cấp đa dạng các APIs và các hàm callback để truy xuất trạng thái của đồ thị. Bằng cách đó, người dùng có thể cập nhật lại đồ thị thậm chí sau khi đồ thị đã được thể hiện. Thể hiện một số thành phần được hỗ trợ bởi thư viện C3.js (Hình \ref{fig:c3apis}).
...
%----------------------------------%
\myparagraph{Phương pháp}
Như đã trình bày ở trên, C3.js sử dụng thư viện D3.js nhằm tạo các đồ thị có khả năng tái sử dụng, cũng như giúp người dùng đào sâu hơn vào khái niệm trực quan dữ liệu thông qua đồ thị trên nền ứng dụng web.
...
%----------------------------------%
\myparagraph{Giới thiệu}
D3.js được viết bởi \textit{Mike Bostock}, là một thư viện Javascript dùng để thao tác với \textit{document} dựa trên dữ liệu. D3.js giúp trực quan dữ liệu bằng cách sử dụng HTML, SVG, và CSS. Sức mạnh của D3.js trên chuẩn web cung cấp đầy đủ các tính năng của những trình duyệt hiện đại mà không buộc mình vào một khuôn khổ nhất định nào, kết hợp với các thành phần trực quan mạnh mẽ và một phương pháp hướng dữ liệu để thao tác DOM (\textit{Document Object Model}).
...
\begin{figure}[!h]
    ...
    \caption{Kết quả Unit Testing của C9js được Istanbul tổng hợp}
...
\begin{figure}[!h]
    ...
    \caption{Trạng thái các dòng lệnh C9js được \textit{cover} trên GitHub, báo cáo bởi công cụ Istanbul}
...
%----------------------------------%
\myparagraph{Giới thiệu}
Như đã giới thiệu sơ qua về thuật ngữ \textit{code coverage} ở mục \ref{sec:istanbul} (\textit{Istanbul}), ở phần này sẽ nói rõ hơn về thuật ngữ này cũng như công cụ để báo cáo mức độ bao phủ mã nguồn, đó là \textit{Codecov.io}.
...
\end{figure}
%----------------------------------%
\subsubsection{Tích hợp và chuyển giao liên tục}
%----------------------------------%
Dưới đây là những phương pháp phát triển phần mềm mà C9js đã sử dụng:

\subsubsubsection{Tích hợp liên tục (\textit{Continuous Integration})}
...
%----------------------------------%
\subsubsubsection{Chuyển giao liên tục (\textit{Continuous Delivery})}

\myparagraph{Khái niệm}
Theo \textit{Martin Fowler}\cite{cd}, Chuyển giao liên tục là nguyên tắc phát triển phần mềm mà ở đó phần mềm có thể được phát hành ở bất kì thời gian nào.
...
\myparagraph{Giới thiệu}
Travis CI là hệ thống hiện thực tích hợp liên tục (\textit{CI}) nhanh chóng và gọn lẹ, tích hợp với \textit{Github, GitLab, Bitbucket}.

\myparagraph{Tính năng}
Các tính năng chính của Travis CI: 
\begin{itemize}
\item[•] Theo dõi các test khi các test này chạy.
\item[•] Giữ các cấu hình (\textit{config}) cùng với mã nguồn của nhà phát triển.
\item[•] Hỗ trợ thông báo qua các kênh: \textit{Slack, HipChat, Email,...}
\item[•] Một máy ảo hoàn toàn mới mỗi khi biên dịch (\textit{build}).
\item[•] Chạy các test song song.
\item[•] Hỗ trợ thêm các hệ điều hành Linux, Mac (và iOS).
\item[•] Có \textit{command line} và \textit{API} mạnh mẽ.
...
\begin{figure}[!h]
	\begin{center}
    \fbox{\includegraphics[scale=.55]{image/advance_1}}
